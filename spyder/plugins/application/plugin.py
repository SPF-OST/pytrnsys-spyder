...
class Application(SpyderPluginV2):
    ...
    def create_new_file(self) -> None:
        ...
        If the plugin that currently has focus, has its `CAN_CREATE_NEW_FILE`
        attribute set to `True`, then create a new file in that plugin.
        Otherwise, create a new file in the Editor plugin.
        """
        plugin = self.get_focused_plugin()
        if plugin and getattr(plugin, 'CAN_CREATE_NEW_FILE', False):
